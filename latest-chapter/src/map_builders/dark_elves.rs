...
pub fn dark_elf_city(new_depth: i32, width: i32, height: i32) -> BuilderChain {
    ...
    chain.with(AreaStartingPosition::new(XStart::Right, YStart::Middle));
    chain.with(AreaEndingPosition::new(XEnd::Left, YEnd::Middle));
...
fn spawn_zones(build_data: &mut BuilderMap) {
    ...
    zone_sizes
        ...
        .for_each(|(i, (zone, _))| match i {
            0 => portal_park(build_data, &voronoi_membership, *zone, &voronoi_seeds),
            1 | 2 => park(build_data, &voronoi_membership, *zone, &voronoi_seeds),
            i if i > 20 => {
                fill_zone(build_data, &voronoi_membership, *zone, TileType::Wall);
            }
            _ => {
                let roll = crate::rng::roll_dice(1, 6);
                match roll {
                    1 => fill_zone(build_data, &voronoi_membership, *zone, TileType::DeepWater),
                    2 => fill_zone(
                        build_data,
                        &voronoi_membership,
                        *zone,
                    ...
                    3 => stalactite_display(build_data, &voronoi_membership, *zone),
                    _ => {}
                }
            }
        });
    ...
    // Clear the path
    make_roads(build_data, &voronoi_membership);
}

fn empty_map(build_data: &mut BuilderMap) {
    build_data
        .map
        .tiles
        .iter_mut()
...
fn stalactite_display(build_data: &mut BuilderMap, voronoi_membership: &[i32], zone: i32) {
    voronoi_membership
        .iter()
        .enumerate()
        .filter(|(_, tile_zone)| **tile_zone == zone)
...
fn make_roads(build_data: &mut BuilderMap, voronoi_membership: &[i32]) {
